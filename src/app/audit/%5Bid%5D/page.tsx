...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (res.success && res.data) {
    ...
    return {
      title: `AI Spend Audit Report | Save $${savings}/yr | Credex`,
      ...
      <div className="min-h-screen flex items-center justify-center p-6 text-center">
        <div className="glass-panel rounded-3xl p-8 max-w-md w-full border-rose-500/20 bg-rose-950/10">
          ...
          <p className="text-gray-400 text-sm mb-6">
            The requested audit ID is invalid or has expired. Let&apos;s run a fresh audit to discover your savings.
          </p>
          <Link
            href="/"
  ...
  return (
    <main className="min-h-screen flex flex-col justify-between">
      ...
      <header className="w-full border-b border-white/5 bg-gray-950/20 py-4 z-10">
        <div className="max-w-6xl mx-auto px-4 md:px-6 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500 border-l border-white/10 pl-2">Cost Report</span>
        </div>
      </header>

      {/* Main client component */}
      <div className="flex-grow">
        <AuditResultClient audit={res.data} />
      </div>
